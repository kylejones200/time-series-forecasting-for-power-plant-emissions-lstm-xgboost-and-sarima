...
# Train final model
sarima = SARIMAX(train_data["total_co2_tons"], order=best_params)
...
def main():
    ...
    for name, model in [
        ...
        rmse = (
            model["rmse"]
            if "rmse" in model
            else np.sqrt(mean_squared_error(model["actuals"], model["predictions"]))
        )
        logger.info(
            f"{name:<15} {model['mae'] / 1e9:<15.3f} {rmse / 1e9:<15.3f} {improvement:+.1f}%"
        )
