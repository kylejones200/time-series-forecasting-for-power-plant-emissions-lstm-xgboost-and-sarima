...
def load_data(cfg: Config) -> pd.DataFrame:
    ...
    if not p.exists():
        raise FileNotFoundError(
            "Power plant emissions CSV not found. See DATA_PowerPlant_Emissions.md for required columns and sample."
        )
    df = pd.read_csv(p)
...
def fit_eval_rf(df: pd.DataFrame, cfg: Config, plot: bool = False):
    ...
    if plot:
        ...
        plt.legend()
        plt.title("RandomForest (lag features) – Chrono CV last fold")
        plt.xlabel("Time")
...
def fit_eval_sarimax(df: pd.DataFrame, cfg: Config, order=(1, 1, 1), seasonal_order=(0, 0, 0, 0)):
    ...
    if plot:
        ...
        plt.legend()
        plt.title("SARIMAX – Chronological holdout")
        plt.xlabel("Time")
        plt.ylabel(cfg.target_col)
