...
def generate_load_curve(base_load_mw=9300, date=None):
    ...
    for hour in range(24):
        load_factor = np.select(
            [6 <= hour <= 9, 17 <= hour <= 21, hour >= 22 or hour <= 5],
            [
                0.85 + np.random.uniform(-0.03, 0.03),
                0.95 + np.random.uniform(-0.03, 0.03),
...
def main():
    ...
    logger.info(
        f"   Load Increase: {(hot_metrics['peak_load_mw'] / base_metrics['peak_load_mw'] - 1) * 100:.1f}%"
    )
    ...
    logger.info(
        f"Forecasts Generated: {len(base_forecast) + len(hot_weather_forecast) + len(week_forecast)}"
    )
